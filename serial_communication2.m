...
if get(hObject, 'value')
    ...
    %% 获取波特率
    rates = [300 600 1200 2400 4800 115200 9600];
...
if hasData
    ...
    %% 若要显示的字符串长度超过100000000，清空显示区
    if length(strRec) > 100000000
        strRec = '';
...
% 
% fid=fopen('ysw.txt','r'); %以只读方式打开文件
% value=textscan(fid,'%s'); %读取其中的数据

global value3

%xlswrite('output.xlsx',value3);
...
set(get(AX(2),'Ylabe1'),'String','Fast Decay') %右侧y轴
xlabel('Time (\musec)')
set(H1,'LineStyle','--')%对应第一条曲线的线性y1
set(H2,'LineStyle',':')%对应第一条曲线的线性y2 


function derive_Callback(hObject, eventdata, handles)
% hObject    handle to derive (see GCBO)
...
%        val=[val,hex2dec(strcat(value2{ii},value2{ii+1}))];%转换
  ...
  %  plot(val,'DisplayName','val','YDataSource','val');figure(gcf)
