...
%画图
[XX YY ZZ]=tubemesh([pt(1,:)',pt(2,:)',pt(3,:)'],0.8);
cdata=cat(3,ones(size(XX)),zeros(size(YY)),zeros(size(ZZ)));%红色
surf(XX,YY,ZZ,cdata);
...
l6=A(1,6);
d=1;
k1=2*((l4^2+l5^2+l6^2-l4*l5-l5*l6-l4*l6)^0.5)/(d*(l4+l5+l6)); 
L1=(l4+l5+l6)/3;
Q1=k1*L1
fi1=atan((3^0.5)/3*(l6+l5-2*l4)/(l5-l6));
 if fi1>0
     if l5>l4
         fi1=fi1;
      else
...
%末端坐标
 a1=cos(fi1)*(1-cos(L1*k1))/k1;
 b1=sin(fi1)*(1-cos(L1*k1))/k1;
 c1=-sin(L1*k1)/k1;
 %求圆心
 [x,y,z]=solve('(x)^2+(y)^2+(z)^2=(1/k1)^2','(x-a1)^2+(y-b1)^2+(z-c1)^2=(1/k1)^2','y=x*tan(fi1)','x','y','z');
 x=eval(x);
 y=eval(y);
 z=eval(z);
 if abs(z(2,1))>abs(z(1,1))
 ...
 if c1>0
   z1=0;
   [x1,y1]=solve('(x1-x)^2+(y1-y)^2+(z1-z)^2==(1/k1)^2','y1=x1*tan(fi1)','x1','y1');
   x1=eval(x1);
   y1=eval(y1);
   X1=abs(x1(1,1));
   Y1=abs(y1(1,1));
...
%坐标变换
 Q=[x1;y1;z1];
 E=[a1;b1;c1];
 R=[((cos(fi))^2)*(cos(k*l)-1)+1 sin(fi)*cos(fi)*(cos(k*l)-1) cos(fi)*sin(-k*l); sin(fi)*cos(fi)*(cos(k*l)-1) ((sin(fi))^2)*(cos(k*l)-1)+1 sin(fi)*sin(-k*l);-cos(fi)*sin(-k*l) -sin(fi)*sin(-k*l) cos(k*l)]
 Q=[a;b;c]+R*Q
 E=[a;b;c]+R*E
 a2=E(1,1);
 b2=E(2,1);
...
%画图
[XX YY ZZ]=tubemesh([pt(1,:)',pt(2,:)',pt(3,:)'],0.8);
cdata=cat(3,zeros(size(XX)),ones(size(XX)),zeros(size(XX)));%绿色
surf(XX,YY,ZZ,cdata);
%tubemesh([pt(1,:)',pt(2,:)',pt(3,:)'],1);
%plot3(pt(1,:),pt(2,:),pt(3,:), 'LineWidth',1);
shading interp;
